...
const MessgeBox = ({ chats }: ChatProps) => {
    return (
        <div className='flex gap-3 max-w-max'>
            ...
            <div>
                <span className='font-bold mb-1'>{chats.role === "user" ? "You" : "Chatley"}</span>
                <ReactMarkdown
                    className="flex flex-col gap-4"
                    components={{
                        code({ children, inline, className, ...props }: any) {
                            const match = /language-(\w+)/.exec(className || '');
                            const language = match && match[1] ? match[1] : "jsx";
                            ...
                            return !inline && match ? (
                                ...
                                <code className="bg-gray-800 text-white px-2 py-[1px] leading-loose rounded font-normal" {...props}>
                                    {children}
                                </code>
                            );
                        }
